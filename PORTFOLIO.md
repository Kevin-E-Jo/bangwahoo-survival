...
- **2026-09-05** — 신규 공격 패턴 3종(부비트랩·자동 딱총·폭탄 던지기) 완료 —
  [PR #12](https://github.com/Kevin-E-Jo/bangwahoo-survival/pull/12). 전부
  자동 발동, 라이브로 지뢰 접촉 데미지·터렛 사거리 게이팅·폭탄 광역데미지
  함수까지 확인. 몹 증가(#10)·월드 확장(#11)과 함께 3중 통합 재검증 —
  확장된 월드에서 새 업그레이드가 실제 선택지로 뜨고, 늘어난 몹 구성(자폭몹
  포함)과 함께 정상 동작하는 것까지 직접 확인.
- **2026-09-05** — 카메라 추적 + 월드 확장 완료 —
  [PR #11](https://github.com/Kevin-E-Jo/bangwahoo-survival/pull/11). 월드
  2400×1350(뷰포트 960×540 유지), 맵 4종 재배치, 엄폐물이 넓은 공간에 고르게
  분산. 담당 세션은 인증 우회 없이 라이브 검증을 스스로 중단했고(안전장치
  올바르게 존중), 대신 제가 직접 라이브로 재검증 — 카메라가 실제로 플레이어를
  따라가는 것, 월드 경계에서 정확히 클램프되는 것(scroll 1440/810 = 2400-960/
  1350-540), 장애물 8개가 넓게 분산된 것까지 확인 완료.
- **2026-09-05** — 몹 증가·자폭몹·속도·드랍률 완료 —
  [PR #10](https://github.com/Kevin-E-Jo/bangwahoo-survival/pull/10). 라운드별
  마릿수 5~8마리로 증가, 기존 4유형 속도 ×1.15, 신규 자폭몹 "폭탄돌리기"(플레이어
  40px 이내 접근 시 예열 후 광역폭발, 총에 맞아 죽으면 폭발 없음), 아이템
  드랍률 ×0.7. 라이브 검증 완료(폭발 데미지·조건부 무폭발까지 확인).
- **2026-09-05** — 전투 다양화 초안(몹 원거리 공격 5종 / 몹 증가·자폭몹·속도·
  드랍률 / 신규 공격 패턴 3종 / 카메라 추적+월드 확장)을 4개 하위 worktree로
  나눠 병렬 구현 착수(`feature/combat-ranged`, `-mob-scaling`, `-new-patterns`,
  `-world`). 전부 `feature/combat` 베이스 PR로 귀결 예정, 병합·충돌 조정은
  완료되는 대로 진행.
- **2026-09-05** — 확정된 설계 중 트리비얼한 3건 즉시 반영: 멀티샷 짝수 발
  중앙 미스 버그 수정(라이브 검증 — 2발 다 중앙 타겟에 명중 확인), 업그레이드
  캡슐에 이름 바로 표시(가챠 "?" 제거), 마을 영구 업그레이드 최대 레벨
  10·성장률 1.75로 재조정(실측 총비용 약 7,156 = 약 130런).
- **2026-09-05** — 대기 중이던 두 설계 초안(전투 다양화·월드 확장 / 스테이지
  셀렉트)의 열린 질문 전부 확정. 스테이지 하나 = 기존 3라운드 그대로(지금까지
  플레이한 건 전부 "1-1"), 종료 시 5초 타임아웃 다이얼로그(다음 스테이지/마을)
  신규 확정. 진행 순서: 전투 다양화 먼저 → 스테이지 셀렉트. 둘 다 설계 완료,
  구현 착수.
- **2026-09-05** — 스테이지 셀렉트 시스템("1-1~1-10, 2-1~2-10...") 설계 초안
  추가 — 지금까지 중 가장 큰 구조 변경. 시드 랜덤 3라운드 런을 챕터·스테이지로
  고정된 맵/몹/난이도 레벨로 교체하는 안. 처음으로 auth-backend·town-scene·
  combat 세 영역 전부를 건드리는 확장이라 원래 3세션 병렬 구조가 다시 맞는
  작업. 구현 전, 첨삭 대기. ([docs/blueprint.html](./docs/blueprint.html#stages))
...
- **2026-09-05** — 업그레이드 시스템 구현 착수. 몹 난이도 상향(체력/접촉데미지
  ...
  나머지(선택 UI·발사패턴·스탯 강화 / 속성탄 9종 상태이상)는 공유 타입
  계약(`upgradeTypes.ts`)을 먼저 고정해두고 `feature/combat-upgrades-core`,
  `feature/combat-elements` 두 worktree로 병렬 진행 중.
- **2026-09-05** — 속성탄 후보 7종(찌릿탄·바람개비탄 포함) 전부 채택 확정 —
  총 9종. 각각 수치·상호 비중복 설계까지 완료, 구현 순서(폭죽·물풍선·찌릿
  먼저 구조 검증 후 나머지)까지 제안. 업그레이드 시스템 설계는 이제
  완전히 확정 — 구현만 남음.
- **2026-09-05** — 속성탄 확장 후보 5종(얼음땡탄·자석탄·불량식품탄·플래시탄·
  호루라기탄) 추가, 전기·바람 속성도 찌릿탄·바람개비탄으로 개명해 이름 톤
  통일. 7종 다 채택 여부는 아직 미정 — 문서 초안 단계.
- **2026-09-05** — 업그레이드 시스템 설계 확정(구현 전): 속성탄은 스택형(불+물
  동시 적용), 도탄은 엄폐물에 튕기는 방식, 선택 UI는 가챠캡슐 여는 연출,
  스탯 강화계는 5스택 상한. 업그레이드 이름 전부 그 시절 느낌으로 교체
  (예: 멀티샷→고무줄 연사, 데미지→왕구슬, 도탄→비석치기). 남은 미확정은
  전기·바람 속성(구현 여부 자체가 미정)뿐 — 설계 문서 기준으로는 사실상 완료.
- **2026-09-05** — 몹 난이도 상향안 세부 수치 확정: 방어력은 전체 공통 2
  (엘리트 포함), 접촉 데미지도 체력과 함께 ×1.5, 엘리트만 ×1.75로 별도
  스케일. 설계 문서 기준 확정 — 구현은 아직.
- **2026-09-05** — 업그레이드 설계 첨삭 반영: 선택 횟수 런당 3회로 확정(라운드
  안 늘림). 몹 난이도 상향안 추가 — 전체 체력 ×1.5, 신규 스탯 "기본
  방어력"(총알 데미지 고정 경감, 최소 1 데미지 보장). 둘 다 아직 설계 단계.
