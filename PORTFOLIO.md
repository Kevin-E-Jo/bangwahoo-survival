...
- **2026-09-05** — 몹 원거리 공격 5종(일반 딱지·탱크 돌멩이·스피드 새총·롤러
  없음·엘리트 책장 부채꼴 5발) 완료, 전투 다양화 4개 하위 브랜치
  (`-ranged`/`-mob-scaling`/`-world`/`-new-patterns`)가 전부 `feature/combat`에
  통합 —
  [PR #13](https://github.com/Kevin-E-Jo/bangwahoo-survival/pull/13). 병합
  과정에서 먼저 병합된 세 브랜치(#10/#11/#12)와의 충돌 4건을 처리(전부
  스프라이트 키·팔레트·필드 추가가 겹친 것뿐이라 양쪽 다 유지). 이어서 직접
  라이브 검증하던 중 실제 버그 발견: `enemyRanged.ts`가 순환 import를 피하려고
  따로 들고 있던 캔버스 크기 상수(960×540)를 투사체 정리(화면 밖 청소) 기준으로
  그대로 쓰고 있었는데, `-world` 브랜치가 이미 병합되며 실제 월드가 2400×1350로
  커진 뒤였다 — 그 결과 적 투사체가 스폰 직후 거의 즉시 청소되어 원거리
  공격이 사실상 전혀 작동하지 않는 상태였다(정지 투사체가 단 1틱만에 사라지는
  것으로 재현·확인). 로컬 상수를 월드 크기로 교체해 수정, 다시 라이브로
  투사체가 캔버스 범위 밖에서도 여러 틱 살아남는 것까지 재확인 후 병합.
  개별 브랜치 검증이나 서브에이전트 보고만으론 잡을 수 없었던, 순수 브랜치 간
  통합 버그.
